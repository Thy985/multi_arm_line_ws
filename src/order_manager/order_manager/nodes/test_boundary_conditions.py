...
def test_concurrent_zone_requests(coord, result, timeout):
    ...
    # left_arm完成后，right_arm应该被触发
    time.sleep(1.0)
    assert coord.arm_status['right_arm'].state == ArmState.WORKING, \
        f"right_arm应该是WORKING状态，实际是{coord.arm_status['right_arm'].state.name}"
...
def test_rapid_commands(coord, result, timeout):
    ...
    # 快速发送多个命令
    success1 = coord.send_to_position('left_arm', 'ready', duration=2.0)
    assert success1, "第一个命令应该成功"
    
    # 立即发送第二个命令 - 应该被拒绝
    success2 = coord.send_to_position('left_arm', 'extended')
...
def test_zero_duration(coord, result, timeout):
    ...
    # 发送零持续时间命令
    success = coord.send_to_position('left_arm', 'ready', duration=0.0)
...
def test_negative_duration(coord, result, timeout):
    ...
    success = coord.send_to_position('left_arm', 'ready', duration=-1.0)
    # 注意：这个测试可能根据实现不同而不同
    # 如果实现不处理负数，可能会失败
    if success:
        # 等待完成
...
def test_goal_cancellation(coord, result, timeout):
    ...
    # left_arm进入zone_a
    granted = coord.send_to_zone('left_arm', 'zone_a', 'ready', duration=5.0)
    assert granted, "left_arm应该获得zone_a"
    time.sleep(0.3)
    assert coord.arm_status['left_arm'].state == ArmState.WORKING
    
    # 手动模拟goal取消（通过设置ERROR状态）
    coord.arm_status['left_arm'].state = ArmState.ERROR
    coord._release_zone_only('left_arm')
    ...
    # 验证left_arm保持ERROR状态
    assert coord.arm_status['left_arm'].state == ArmState.ERROR, \
        f"left_arm应该是ERROR状态，实际是{coord.arm_status['left_arm'].state.name}"
    
    print("    Goal取消: zone被释放，arm保持ERROR状态")
    
    # 清理：重置left_arm
    coord.reset_arm('left_arm')
...
def test_timeout_auto_cancel(coord, result, timeout):
    ...
    # 等待正常完成（超时测试需要特殊处理）
    # 这里我们测试超时逻辑是否存在
    status = coord.arm_status['left_arm']
...
def test_error_state_persistence(coord, result, timeout):
    ...
    # 手动设置ERROR状态
    coord.arm_status['left_arm'].state = ArmState.ERROR
    coord.arm_status['left_arm'].error_message = "测试错误"
...
def test_non_error_state_reset(coord, result, timeout):
    ...
    # 发送命令使arm变为WORKING
    coord.send_to_position('left_arm', 'ready', duration=2.0)
    time.sleep(0.3)
    assert coord.arm_status['left_arm'].state == ArmState.WORKING
...
def test_queued_arm_trigger(coord, result, timeout):
    """排队臂触发：left_arm完成后自动触发right_arm"""
    # 确保干净状态
    assert coord.arm_status['left_arm'].state == ArmState.IDLE
    assert coord.arm_status['right_arm'].state == ArmState.IDLE
    ...
    # right_arm也请求zone_a - 应该排队
    granted2 = coord.send_to_zone('right_arm', 'zone_a', 'ready', duration=3.0)
    assert not granted2, "right_arm应该被排队"
    assert coord.arm_status['right_arm'].state == ArmState.QUEUED, \
        f"right_arm应该是QUEUED状态，实际是{coord.arm_status['right_arm'].state.name}"
    
    # 记录right_arm的requested_position
    requested_position = coord.arm_status['right_arm'].requested_position
    assert requested_position == 'ready', \
        f"right_arm的requested_position应该是ready，实际是{requested_position}"
    
    print(f"    right_arm requested_position: {requested_position}")
    
    # 等待left_arm完成
    wait_for_state_change(coord, 'left_arm', ArmState.WORKING, timeout=timeout)
    
    # left_arm完成后，right_arm应该被触发并移动到zone_a
    time.sleep(1.0)
    assert coord.arm_status['right_arm'].state == ArmState.WORKING, \
        f"right_arm应该是WORKING状态，实际是{coord.arm_status['right_arm'].state.name}"
