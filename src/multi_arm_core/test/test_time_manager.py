...
class TestTimeManager:
    ...
    def test_schedule_no_conflict(self) -> None:
        tm = TimeManager()
        result = tm.schedule("left_arm", "zone_a", duration=3.0)
    ...
    def test_schedule_conflict_same_zone(self) -> None:
        ...
        result = tm.schedule("right_arm", "zone_a", duration=3.0)
    ...
    def test_no_conflict_different_zone(self) -> None:
        tm = TimeManager()
        tm.schedule("left_arm", "zone_a", duration=3.0)
        result = tm.schedule("right_arm", "zone_b", duration=3.0)
        assert result.granted

    def test_no_conflict_same_arm(self) -> None:
        tm = TimeManager()
        tm.schedule("left_arm", "zone_a", duration=3.0)
        result = tm.schedule("left_arm", "zone_a", duration=3.0, start_delay=5.0)
        assert result.granted

    def test_cancel(self) -> None:
        tm = TimeManager()
        tm.schedule("left_arm", "zone_a", duration=3.0, start_delay=10.0)
        assert tm.cancel("left_arm")
    ...
    def test_get_arm_end_time(self) -> None:
        ...
        now = tm.now()
        tm.schedule("left_arm", "zone_a", duration=3.0)
        end_time = tm.get_arm_end_time("left_arm")
        assert end_time > now
    ...
    def test_schedule_with_delay_avoids_conflict(self) -> None:
        tm = TimeManager()
        result1 = tm.schedule("left_arm", "zone_a", duration=3.0)
        assert result1.granted

        result2 = tm.schedule("right_arm", "zone_a", duration=3.0)
        assert not result2.granted

        result3 = tm.schedule(
            "right_arm", "zone_a", duration=3.0, start_delay=result2.suggested_delay + 0.1
        )
