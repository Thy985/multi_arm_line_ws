"""M6 Skill Showcase — real robot motion driven through the Skill layer.

This is the single artifact that demonstrates the CURRENT (M6) real capability
end-to-end. It proves the simulator chain still works after the M6 architecture
upgrade by driving Gazebo through the full M6 skill stack:
...
This differs from pre-M6 scripts (m4_6/m5_6) which called ExecuteTask directly.
Here the Coordinator is reached through the M6 Skill Runtime + Robot Runtime API.
"""
...
def launch_setup(context, *args, **kwargs):
    ...
    arm1_jtc_spawner = Node(
        package="controller_manager",
        executable="spawner",
        arguments=["arm1_joint_trajectory_controller", "-c", "/controller_manager"],
        parameters=[ParameterFile(controllers_config, allow_substs=True)],
    )
    arm2_jtc_spawner = Node(
        package="controller_manager",
        executable="spawner",
        arguments=["arm2_joint_trajectory_controller", "-c", "/controller_manager"],
        parameters=[ParameterFile(controllers_config, allow_substs=True)],
    )
    ...
    system_nodes = [
        ("multi_arm_world_model", "world_model_node", 6.0),
        ("multi_arm_safety", "safety_supervisor", 6.0),
        ("multi_arm_core", "coordinator_node", 8.0),
        ("multi_arm_task_planner", "task_planner_node", 10.0),
        ("multi_arm_robot_description", "capability_registry_node", 6.0),
        ("multi_arm_experience", "experience_node", 6.0),
    ]
    for pkg, exe, delay in system_nodes:
        n = Node(package=pkg, executable=exe, output="screen",
                 parameters=[{"use_sim_time": True}])
        nodes.append(TimerAction(period=delay + 2.0, actions=[n]))

    skill_node = Node(
        package="multi_arm_skill_runtime",
        executable="skill_node",
        output="screen",
        parameters=[{"use_sim_time": True}, {"use_real_motion": True}],
    )
    nodes.append(TimerAction(period=13.0, actions=[skill_node]))
    ...
    nodes.append(TimerAction(period=14.0, actions=[runtime_api_node]))

    showcase_script = os.path.join(
        moveit_dir, "scripts", "m6_skill_showcase_e2e.py"
    )
    showcase_cmd = ExecuteProcess(
        cmd=["python3", showcase_script], output="screen",
    )
    nodes.append(TimerAction(period=30.0, actions=[showcase_cmd]))

    return nodes


def generate_launch_description() -> LaunchDescription:
    """Generate the showcase launch description."""
