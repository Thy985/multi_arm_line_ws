...
class TestWorkspaceLimiter:
    ...
    def test_default_bounds_check(self) -> None:
        ...
        within, dist = limiter.check_position("arm1", 0.0, 0.0, 0.5)
        assert within

    def test_out_of_bounds(self) -> None:
        limiter = WorkspaceLimiter()
        within, dist = limiter.check_position("arm1", 1.5, 0.0, 0.5)
        assert not within
    ...
    def test_per_arm_bounds(self) -> None:
        ...
        within1, _ = limiter.check_position("arm1", 0.0, 0.6, 0.5)
        within2, _ = limiter.check_position("arm2", 0.0, 0.6, 0.5)
        assert within1
        assert not within2
