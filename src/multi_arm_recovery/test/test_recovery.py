...
class TestFailureClassifier:
    ...
    def test_classify_planning_failure_moveit_error(self):
        event = self.classifier.classify("moveit_error_-1", arm_name="left_arm")
        assert event.failure_type == FailureType.PLANNING_FAILURE
        assert event.recoverable is True

    def test_classify_planning_failure_goal_send_timeout(self):
        event = self.classifier.classify("goal_send_timeout", arm_name="left_arm")
        assert event.failure_type == FailureType.PLANNING_FAILURE

    def test_classify_planning_failure_goal_rejected(self):
        event = self.classifier.classify("goal_rejected", arm_name="left_arm")
        assert event.failure_type == FailureType.PLANNING_FAILURE

    def test_classify_collision(self):
        event = self.classifier.classify("collision detected", arm_name="left_arm")
    ...
    def test_classify_collision_from_context(self):
        event = self.classifier.classify(
            "motion failed", arm_name="left_arm", context={"collision_detected": True}
        )
        assert event.failure_type == FailureType.COLLISION_DETECTED

    def test_classify_safety_rejection(self):
        event = self.classifier.classify("Safety check rejected", arm_name="left_arm")
        assert event.failure_type == FailureType.SAFETY_REJECTION
        assert event.recoverable is False

    def test_classify_e_stop(self):
        event = self.classifier.classify("E-Stop active", arm_name="left_arm")
        assert event.failure_type == FailureType.SAFETY_REJECTION
        assert event.recoverable is False

    def test_classify_zone_occupied(self):
        event = self.classifier.classify("Zone zone_a occupied", arm_name="left_arm")
        assert event.failure_type == FailureType.RESOURCE_TIMEOUT

    def test_classify_jtc_failed(self):
        event = self.classifier.classify("jtc_failed", arm_name="left_arm")
        assert event.failure_type == FailureType.CONTROLLER_FAILURE

    def test_classify_execution_timeout(self):
        event = self.classifier.classify("execution_timeout", arm_name="left_arm")
        assert event.failure_type == FailureType.EXECUTION_TIMEOUT

    def test_classify_grasp_failure(self):
        event = self.classifier.classify("grasp failed", arm_name="left_arm")
        assert event.failure_type == FailureType.GRASP_FAILURE

    def test_classify_grasp_from_context(self):
        event = self.classifier.classify(
            "action failed", arm_name="left_arm", context={"grasp_failed": True}
        )
        assert event.failure_type == FailureType.GRASP_FAILURE

    def test_classify_resource_timeout_from_context(self):
        event = self.classifier.classify(
            "wait failed", arm_name="left_arm", context={"resource_timeout": True}
        )
        assert event.failure_type == FailureType.RESOURCE_TIMEOUT

    def test_classify_unknown(self):
        event = self.classifier.classify("something weird happened", arm_name="left_arm")
        assert event.failure_type == FailureType.UNKNOWN
        assert event.recoverable is True

    def test_classify_preserves_arm_name(self):
        event = self.classifier.classify("jtc_failed", arm_name="right_arm")
        assert event.arm_name == "right_arm"

    def test_classify_preserves_task_id(self):
        event = self.classifier.classify("jtc_failed", arm_name="left_arm", task_id="t123")
        assert event.task_id == "t123"

    def test_classify_preserves_context(self):
        ctx = {"zone": "zone_a", "error_code": -1}
        event = self.classifier.classify("jtc_failed", arm_name="left_arm", context=ctx)
...
class TestPlanningFailureHandler:
    ...
    def test_can_handle_planning_failure(self):
        event = FailureEvent(failure_type=FailureType.PLANNING_FAILURE, arm_name="left_arm")
        assert self.handler.can_handle(event) is True

    def test_cannot_handle_other_failure(self):
        event = FailureEvent(failure_type=FailureType.COLLISION_DETECTED, arm_name="left_arm")
...
class TestResourceTimeoutHandler:
    ...
    def test_first_strategy_release_and_requeue(self):
        event = FailureEvent(failure_type=FailureType.RESOURCE_TIMEOUT, arm_name="left_arm")
        name, params = self.handler.get_recovery_strategy(event)
...
class TestControllerFailureHandler:
    ...
    def test_first_strategy_wait_and_retry(self):
        event = FailureEvent(failure_type=FailureType.CONTROLLER_FAILURE, arm_name="left_arm")
        name, params = self.handler.get_recovery_strategy(event)
...
class TestGraspRetryHandler:
    ...
    def test_first_retry(self):
        event = FailureEvent(failure_type=FailureType.GRASP_FAILURE, arm_name="left_arm")
        name, params = self.handler.get_recovery_strategy(event)
...
class TestRecoveryManager:
    ...
    def test_classify_failure(self):
        event = self.manager.classify_failure("moveit_error_-1", arm_name="left_arm")
        assert event.failure_type == FailureType.PLANNING_FAILURE
