...
def launch_setup(context, *args, **kwargs):
    ...
    # Note: Camera image is generated by SyntheticCameraNode (not Gazebo bridge)
    # because Gazebo's ogre2 rendering doesn't work in headless mode.
    nodes.append(Node(
    ...
    nodes.append(RegisterEventHandler(
        OnProcessExit(target_action=head_spawner, on_exit=[move_group])
    ))
    ...
    # === Synthetic Camera Node (M7.5: generates images from GT poses) ===
    # In headless mode, Gazebo's ogre2 rendering can't generate camera data.
    # This node creates synthetic camera images by projecting GT poses to 2D,
    # enabling the full ColorDetectorNode OpenCV pipeline without GPU.
    ...
    nodes.append(TimerAction(period=6.0, actions=[synthetic_camera_node]))

    # === Color Detector Node (M7.5: camera image → OpenCV detection → pose) ===
